...
Map::Map(Map &map) :hConsole(map.hConsole), name(map.name)
{
	element = new MapElement *[MAP_WIDTH];
	for (int i = 0; i < MAP_WIDTH; i++)
		element[i] = new MapElement[MAP_HEIGHT];
...
void Map::displayElement(MapElement ele)
{
	gotoXY(ele.pos.X, ele.pos.Y);
	SetConsoleTextAttribute(hConsole, ele.colorIndex);
	std::cout << "¢i ";
